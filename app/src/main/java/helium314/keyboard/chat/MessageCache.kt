...
class MessageCache(private val maxSize: Int = 15) {
    ...
    /**
     * The timestampMs of the most recent message in the buffer, or 0 if empty.
     * Used by ChatBridgeReceiver to skip appending messages that are already
     * represented in the cache (same-chat broadcast re-delivers existing msgs).
     */
    fun lastTimestampMs(): Long = lock.read { buffer.lastOrNull()?.timestampMs ?: 0L }
}
